...
int main() {
  ...
  // Schedule config(5, 6, std::time(nullptr));
  // std::cout << std::time(nullptr) << std::endl;
  Schedule config(5, 6, 1518672559);
  ...
  while (hard_count > 0) hard_count += config.HardSolver(), config.TestPrint();
  config.SoftInitialize();
  config.TestPrint();
  int soft_count = config.SoftCount();
  while (soft_count > 4) soft_count += config.SoftSolver();
  config.TestPrint();
  return 0;
}